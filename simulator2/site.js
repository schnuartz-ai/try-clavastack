...
// Each iframe boots a full MicroPython+WASM runtime (Asyncify-instrumented,
// several MB to compile). Starting all three at once made every visitor's
// browser fight over 1-2 CPU cores for the first ~15-20s. Staggering the
// navigation spreads that compile spike out instead of stacking it.
// On a genuinely constrained device (low RAM and/or few cores - the signal
// we have for "this machine is already under real pressure"), running all
// three at once can starve the tab entirely (silent freeze, no console
// error - the renderer just can't keep up). There, only start device 1
// automatically and let the visitor start the other two when ready.
const constrained = (navigator.deviceMemory && navigator.deviceMemory <= 4) ||
  (navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 2);
function startDevice(name) {
  const el = devices[name].querySelector('.start-overlay');
  if (el) el.remove();
  frames[name].src = frames[name].dataset.src;
}
order.forEach((name, i) => {
  if (constrained && name !== 'diy') {
    const overlay = document.createElement('button');
    overlay.type = 'button';
    overlay.className = 'start-overlay';
    overlay.textContent = 'Tap to start this device';
    overlay.title = 'Deferred on this device to avoid overloading it - tap to start anyway';
    overlay.addEventListener('click', () => startDevice(name));
    devices[name].querySelector('.device-frame').appendChild(overlay);
  } else {
    setTimeout(() => startDevice(name), i * 400);
  }
});
const ready = new Set();
