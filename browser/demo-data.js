...
export function createDemoFiles(primary = 'ghost') {
  ...
  add('00-CLAVASTACK-DEMO-README.txt', [
    ...
    'Testnet wallet files are primary. Mainnet files are only for display comparisons.',
  ].join('\n'));
  for (const [id, root] of Object.entries(roots)) {
    ...
    add(`03-${id}-HOST-IMPORT.txt`, `bip39: ${root.mnemonic}\n`);
  }
  ...
  add('testnet-addresses.txt', wallets.filter(([name]) => name.startsWith('testnet-')).map(([, label, , address]) => `${label}: ${address}`).join('\n') + '\n');
