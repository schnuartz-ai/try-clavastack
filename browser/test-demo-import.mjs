...
if (process.env.TEST_LOCAL_CHANGES === '1') {
  for (const [url, path, contentType] of [
    ...
    [`${base}/browser/demo-data.js?v=20260916-2of3-cards`, 'browser/demo-data.js', 'text/javascript'],
...
await page.locator('#demo-status').getByText('files are on the inserted SD card', { exact: false }).waitFor({ timeout: 30000 });
for (const name of ['01-ghost-PUBLIC-TEST-SEED.txt', '02-zoo-bip85-child-1.txt',
  '03-zoo-HOST-IMPORT.txt', 'testnet-ghost-zoo-mirror-2of3.json',
